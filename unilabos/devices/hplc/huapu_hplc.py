...
class HuapuHPLC:
    ...
    def create_sequence(
        ...
        name: str = "",
        inst_id_arr: Optional[List[int]] = None,
        sample_arr: Optional[List[Dict[str, Any]]] = None,
        seq_tray_num: int = -1,
    ) -> Dict[str, Any]:
        """
        Args:
            proc_inst_id[流程ID]: 用户指定的流程 ID，int 类型，同一项目内必须唯一。
            project_id[项目ID]: 为空或 0 时使用默认项目 ID。
            sequence_method_id[参考序列方法ID]: 为空或 0 时使用默认序列方法 ID。
            name[序列方法名称]: 为空时自动使用 proc_inst_id 生成名称。
            inst_id_arr[仪器ID列表]: 为空时使用默认仪器 ID。
            sample_arr[样品序列]: 华谱 sampleArr 原始结构。
        ...
        inst_id_arr = inst_id_arr or ([self.default_instrument_id] if self.default_instrument_id else [])
        sample_arr = sample_arr or [
            {
                "trayNum": self.default_tray_num,
                "xPosition": self.default_x_position,
                "yPosition": self.default_y_position,
            }
        ]
        payload = {
            "procInstId": int(proc_inst_id),
            "projectId": project_id,
            "sequenceMethodId": sequence_method_id,
            "name": name or f"sequence_{int(proc_inst_id)}",
            "seqTrayNum": seq_tray_num,
            "sampleArr": sample_arr,
            "instIdArr": [int(item) for item in inst_id_arr],
        }
        result = self._post("/project/addSequenceMethod", payload)
        if result["success"]:
            self.data["status"] = "SequenceCreated"
            self.data["last_proc_inst_id"] = int(proc_inst_id)
        return result
    ...
    def control_sequence(self, action: int, proc_inst_id: int = 0) -> Dict[str, Any]:
        """
        Args:
            action[控制动作]: 1-暂停，2-重开，3-停止所有序列。
            proc_inst_id[流程ID]: 调试记录要求复用流程 ID，若服务端忽略该字段也可传 0。
        """
        payload = {"action": int(action)}
        if proc_inst_id:
            payload["procInstId"] = int(proc_inst_id)
    ...
    def get_sequence_status(self, proc_inst_id: int) -> Dict[str, Any]:
        result = self._post("/project/getSequenceStatus", {"procInstId": int(proc_inst_id)})
    ...
    def run_sample(
        ...
        """
        Args:
            proc_inst_id[流程ID]: 用户指定的 int 流程 ID，同一项目内必须唯一。
            project_id[项目ID]: 为空或 0 时使用默认项目 ID。
            sequence_method_id[参考序列方法ID]: 为空或 0 时使用默认序列方法 ID。
            process_method_id[处理方法ID]: 为空或 0 时使用默认处理方法 ID。
            report_method_id[报告方法ID]: 为空或 0 时使用默认报告方法 ID。
        ...
        create_result = self.create_sequence(
            ...
            name=name,
            inst_id_arr=[instrument_id] if instrument_id else None,
            sample_arr=[sample],
        )
        ...
        return self._make_error(f"等待序列完成超时: procInstId={int(proc_inst_id)}", last_status)
    ...
    @property
    @topic_config()
    def last_proc_inst_id(self) -> int:
        return int(self.data.get("last_proc_inst_id", 0))
